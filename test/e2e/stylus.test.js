...
module.exports = {
	...
	"Stylus Tests: Jg-Row Tests": function(browser) {
		...
		browser.expect.element('.jg-row').to.have.css('width').which.equals('1000px');
		// browser.expect.element('.jg-row').to.have.css('margin').which.equals('0 auto');

		browser.resizeWindow(900, 600);
		// browser.expect.element('.jg-row').to.have.css('width').which.equals(900 - (2*8) + 'px');
		browser.expect.element('.jg-row').to.have.css('padding').which.equals('0px 10px');
	},
	"Stylus Tests: Jg-row-1-12 jg-col-1-12 Large": function(browser) {
		browser.resizeWindow(1210, 600);
